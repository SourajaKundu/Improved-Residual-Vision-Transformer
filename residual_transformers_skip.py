...
class Res_CNN(nn.Module):
    ...
    ############################################################################################
        ...
        #self.shortcut1 = nn.Sequential(
            #nn.Conv2d(in_channels=input_dim, out_channels=256, kernel_size=4, stride=4, bias=False),
            #nn.BatchNorm2d(out_channels))
        #self.shortcut2 = nn.Sequential()
        #self.shortcut2 = nn.Sequential(
            #nn.Conv2d(in_channels=256, out_channels=output_dim, kernel_size=4, stride=4, bias=False),
            #nn.BatchNorm2d(out_channels))
    ...
    def forward(self, x):
        ...
        #print('encoder layer 1 output shape:', x.shape)
        x_input=self.skip_1(x_input)
        #print('skip-1 output shape:', x_input.shape)
        x=x+x_input
        #print('final layer-1 shape:', x.shape)
        x1=x
        x = self.encoder_2(x)
        #print('encoder layer 2 output shape:', x.shape)
        x1=self.skip_2(x1)
        #print('skip-2 output shape:', x1.shape)
        x=x+x1
        x2=x
        x = self.encoder_3(x)
        x2=self.skip_3(x2)
